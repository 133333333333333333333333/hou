<template>
  <div class="users">
    ...
    <!-- 修改用户信息对话框 -->
    <el-dialog title="修改用户" @close="editClosed" :visible.sync="editDialogVisble" width="50%">
...
export default {
  ...
  methods: {
    ...
    async userStatuChanged(userInfo) {
      ...
      const { data: res } = await getUsersStatuChangedApi(userInfo.id, userInfo.mg_state)
      if (res.meta.status !== 200) {
    ...
    addUser() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        const { data: res } = await getUsersAdd(this.addForm)
    ...
    async showEditDialog(id) {
      ...
      const { data: res } = await getEditDialog(id)
      if (res.meta.status !== 200) {
    ...
    editUserInfo() {
      this.$refs.editFormRef.validate(async valid => {
        ...
        // })
        const { data: res } = await getEditUserInfo(this.editForm.id, {
          email: this.editForm.email,
          mobile: this.editForm.mobile
        })
    ...
    async setRoles(userInfo) {
      ...
      const { data: res } = await getSetRoles()
      if (res.meta.status !== 200) {
    ...
    async saveRolesInfo() {
      ...
      // })
      const { data: res } = await getSaveRolesInfo(this.userInfo.id, {rid:this.selectRoleId})
      console.log(res)
      if (res.meta.status !== 200) {
