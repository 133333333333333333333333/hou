...
export default {
  ...
  methods: {
    ...
    async getParamsData() {
      ...
      //   }
      // )
      const { data: res } = await getParamsDataApi(this.cateId, this.activeName)
      if (res.meta.status !== 200) {
    ...
    async showEditDialog(id) {
      ...
      // })
      const { data: res } = await showParamsEditDialog(this.cateId,id,  this.activeName)
      console.log(res)
      if (res.meta.status !== 200) {
    ...
    editParams() {
      this.$refs.editFormRef.validate(async valid => {
        ...
        //   attr_sel: this.activeName
        // })
        const { data: res } = await editParamsApi(this.cateId,this.editForm.attr_id, {
          attr_name: this.editForm.attr_name,
          attr_sel: this.activeName
        })
    ...
    async removeParams(id) {
      ...
      const { data: res } = await removeParamsApi(this.cateId,id)
      console.log(res)
      if (res.meta.status !== 200) {
    ...
    async saveAttrVals(row) {
      ...
      // })
      const { data: res } = await saveAttrValsApi(this.cateId,row.attr_id, {
        attr_sel: this.activeName,
        attr_name: row.attr_name,
        attr_vals: row.attr_vals.join(' ')
      })
