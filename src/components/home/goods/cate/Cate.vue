...
import {
  showeditCateDialogApi,
  editCateInfoApi,
  getParentCateListApi,
  getAddCateListApi,
  addCateApi,
  removeCateApi
  } from '@/api/orderApi'
export default {
  ...
  methods: {
    ...
    async getCateList() {
      ...
      // })
      const { data: res } = await getAddCateListApi(this.queryInfo)
    ...
    async getParentCateList() {
      ...
      //   params: { type: 2 }
      // })
      const { data: res } = getParentCateListApi('categories', 2)
      if (res.meta.status !== 200) {
    ...
    async showeditCateDialog(cateInfo) {
      ...
      const { data: res } = await showeditCateDialogApi(cateInfo.cat_id)
    ...
    async editCateInfo() {
      ...
      const { data: res } = await editCateInfoApi(this.editCate.cat_id, { cat_name: this.editCate.cat_name })
      if (res.meta.status !== 200) {
    ...
    async removeCate(id) {
      ...
      const { data: res } = await removeCateApi(id)
      if (res.meta.status !== 200) {
