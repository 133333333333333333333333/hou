...
import { getCateListApi, tabClickedApi, addShopApi } from '@/api/orderApi'
export default {
  name: 'Add',
  components: {
    Breadcrumb,
  ...
  methods: {
    ...
    async tabClicked() {
      ...
      if (this.activeIndex === '1') {
        ...
        // params: {sel: 'many'}
        // })
        const { data: res } = await tabClickedApi(this.cateId, 'many')
        if (res.meta.status !== 200) {
      ...
      if (this.activeIndex === '2') {
        ...
        //   params: { sel: 'only' }
        // })
        const { data: res } = await tabClickedApi(this.cateId, 'only')
        if (res.meta.status !== 200) {
    ...
    addShop() {
      this.$refs.addFormRef.validate(async (valid) => {
        ...
        const { data: res } = await addShopApi(form)
        console.log(res)
