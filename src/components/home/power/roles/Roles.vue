...
import { getRolesListApi, getAddRolesUserApi, getRolesDelete, getShowEditDialogApi, getEditFormInfoApi, getRemoveRightIdApi, getShowSetRightDialog, getAllotRightsApi } from '@/api/orderApi'
export default {
  name: 'Roles',
  components: {
    Breadcrumb
  ...
  methods: {
    ...
    addRolesUser() {
      this.$refs.addRolesForm.validate(async valid => {
        ...
        const { data: res } = await getAddRolesUserApi(this.addRolesForm)
        console.log(res)
    ...
    async showEditDialog(id) {
      ...
      const { data: res } = await getShowEditDialogApi(id)
      if (res.meta.status !== 200) {
    ...
    editFormInfo() {
      this.$refs.editFormRef.validate(valid => {
        ...
        //   })
        getEditFormInfoApi(this.editRolesForm.roleId, {
            roleName: this.editRolesForm.roleName,
            roleDesc: this.editRolesForm.roleDesc
          })
          .then(res => {
    ...
    async removeRightById(roles, id) {
      ...
      const { data: res } = await getRemoveRightIdApi(roles.id,id)
      if (res.meta.status !== 200) {
    ...
    async showSetRightDialog(roles) {
      ...
      const { data: res } = await getShowSetRightDialog()
      if (res.meta.status !== 200) {
    ...
    async allotRights() {
      ...
      const { data: res } = await getAllotRightsApi(this.rolesId, { rids: idStr })
      if (res.meta.status !== 200) {
